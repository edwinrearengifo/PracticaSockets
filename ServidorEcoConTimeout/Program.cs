            ...
            for (; ; )
            {
                ...
                try
                {
                    ...
                    buferRx.Length, SocketFlags.None)) > 0)
                    {
                        ...
                        cliente.SetSocketOption(SocketOptionLevel.Socket,
                        SocketOptionName.ReceiveTimeout, (int)(LIMITE_ESPERA - tiempoTranscurrido.TotalMilliseconds));
                    }
                ...
                catch (SocketException se)
                {
                    ...
                    {
                        Console.WriteLine("Terminado la conexion debido al temporizador.Han transcurrido " + LIMITE_ESPERA + "ms; se hantransmitido " + totalBytesEnviados + " bytes");
                    }
                    else
                    {
                        Console.WriteLine(se.ErrorCode + ": " + se.Message);
